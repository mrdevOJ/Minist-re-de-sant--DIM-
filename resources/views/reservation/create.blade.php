...
@if(Auth::user()->name>0)
  ...
  <form class="row g-3 needs-validation" action="{{route('reservation.store')}}" method="POST" >
    @csrf
  ...
  <li class="list-group-item">Prenom: {{ Auth::user()->prenom }}</li>
   @if(Auth::user()->division !== 'null')
  <li class="list-group-item">Division :  <span class="text-danger">{{ Auth::user()->division }}</span></li>
  <li class="list-group-item">Service :  <span class="text-danger">{{ Auth::user()->service }}</span></li>
    @else
  <li class="list-group-item ">unité : <span class="text-danger">{{ Auth::user()->unité }}</span> </li>
  @endif
</ul>   
...
<input type="hidden" name="unite" value="{{ Auth::user()->unité }}"> 
 @if(count($salles) > 0)
...
<div class="form-error text-danger">
   ...
   <label class="form-label"  for="form5Example2">La Durée</label>
<input class="form-control w-25"  pattern="\b(?:0[89]|1[0-8]):[0-5]\d\b" required type="time" name="first"> 
<div class="form-error text-danger">
           @error('first')
        {{$message}}
        @enderror
        </div>
<input class="form-control w-25"  pattern="\b(?:0[89]|1[0-8]):[0-5]\d\b" required type="time" name="last"> 
<div class="form-error text-danger">
           @error('last')
        {{$message}}
        @enderror
...
<label class="form-label" >La date</label>
    <input type="date" name="date" value="{{old('date')}}"  class="form-control w-25" required autocomplete="new-date" />
    <div class="form-error text-danger">
           @error('date')
        {{$message}}
        @enderror
